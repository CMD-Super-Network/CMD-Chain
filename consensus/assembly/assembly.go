...
// Package assembly implements the proof-of-stake-authority consensus engine.
package assembly
...
import (
	...
	"github.com/ethereum/go-ethereum/consensus/assembly/systemcontract"
...
// New creates a Assembly proof-of-stake-authority consensus engine with the initial
// validators set to the ones provided by the user.
func New(chainConfig *params.ChainConfig, db ethdb.Database) *Assembly {
...
func (c *Assembly) SetStateFn(fn StateFn) {
	c.stateFn = fn
}

// Author implements consensus.Engine, returning the Ethereum address recovered
// from the signature in the header's extra-data section.
func (c *Assembly) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
	// return ecrecover(header, c.signatures)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *Assembly) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (c *Assembly) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (c *Assembly) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Assembly) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Assembly) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *Assembly) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (c *Assembly) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (c *Assembly) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Assembly) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// rewards given.
func (c *Assembly) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction, uncles []*types.Header, receipts *[]*types.Receipt, systemTxs []*types.Transaction) error {
...
func (c *Assembly) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (b *types.Block, rs []*types.Receipt, err error) {
	defer func() {
		if err != nil {
...
// func (c *Congress) getCount(contractAddr *common.Address, method *string) (big.Int, error){
...
func (c *Assembly) trySendBlockReward(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	if err2 != nil {
		log.Error("Can't pack data for adjustRemainderMine", "err2", err2)
		return err2
	}
	totalF := big.NewInt(1000000000000000)
	state.AddBalance(header.Coinbase, totalF)
...
func (c *Assembly) doSomethingAtEpoch(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) ([]common.Address, error) {
	newSortedValidators, err := c.getTopValidators(chain, header)
	if err != nil {
		return []common.Address{}, err
	}
...
func (c *Assembly) initializeSystemContracts(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
...
func (c *Assembly) getTopValidators(chain consensus.ChainHeaderReader, header *types.Header) ([]common.Address, error) {
	...
	// use parent
	result, err := caller.ExecuteMsg(msg, statedb, parent, newChainContext(chain, c), c.chainConfig)
	if err != nil {
		return []common.Address{}, err
	}
...
func (c *Assembly) updateValidators(vals []common.Address, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// method
...
func (c *Assembly) punishValidator(val common.Address, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// method
	method := "punish"
...
func (c *Assembly) decreaseMissedBlocksCounter(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// method
...
// with.
func (c *Assembly) Authorize(validator common.Address, signFn ValidatorFn, signTxFn SignTxFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *Assembly) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
...
func (c *Assembly) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (c *Assembly) SealHash(header *types.Header) common.Hash {
	return SealHash(header)
}

// Close implements consensus.Engine. It's a noop for Assembly as there are no background threads.
func (c *Assembly) Close() error {
	return nil
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the validator voting.
func (c *Assembly) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "congress",
		Version:   "1.0",
		Service:   &API{chain: chain, assembly: c},
		Public:    false,
	}}
}
...
func (c *Assembly) IsSysTransaction(tx *types.Transaction, header *types.Header) (bool, error) {
	if tx.To() == nil {
		return false, nil
	}
	...
	// to := tx.To()
	if sender == header.Coinbase && tx.GasPrice().Sign() == 0 {
		return true, nil
	}
...
func (c *Assembly) ValidateTx(tx *types.Transaction, header *types.Header, parentState *state.StateDB) error {
	// Must use the parent state for current validation,
	// so we must starting the validation after redCoastBlock
	if c.chainConfig.RedCoastBlock != nil && c.chainConfig.RedCoastBlock.Cmp(header.Number) < 0 {
		_, err := types.Sender(c.signer, tx)
		if err != nil {
			return err
		}
	}
...
func (c *Assembly) PreHandle(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	if c.chainConfig.RedCoastBlock != nil && c.chainConfig.RedCoastBlock.Cmp(header.Number) == 0 {
