...
import (
	...
	"github.com/ethereum/go-ethereum/consensus/assembly"
...
func New(stack *node.Node, config *Config) (*Ethereum, error) {
	...
	// do some extra work if consensus engine is assembly.
	if assemblyEngine, ok := eth.engine.(*assembly.Assembly); ok {
		// set state fn
		assemblyEngine.SetStateFn(eth.blockchain.StateAt)
		// set consensus-related transaction validator
		eth.txPool.InitExTxValidator(assemblyEngine)
	}
...
func CreateConsensusEngine(stack *node.Node, chainConfig *params.ChainConfig, config *ethash.Config, notify []string, noverify bool, db ethdb.Database) consensus.Engine {
	...
	if chainConfig.Congress != nil {
		return assembly.New(chainConfig, db)
	}
...
func (s *Ethereum) shouldPreserve(block *types.Block) bool {
	...
	if _, ok := s.engine.(*assembly.Assembly); ok {
		return false
	}
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		if assembly, ok := s.engine.(*assembly.Assembly); ok {
			wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
			if wallet == nil || err != nil {
				log.Error("Etherbase account unavailable locally", "err", err)
				return fmt.Errorf("signer missing: %v", err)
			}
			assembly.Authorize(eb, wallet.SignData, wallet.SignTx)
		}
